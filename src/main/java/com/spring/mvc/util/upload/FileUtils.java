...
public class FileUtils {
    ...
    public static String uploadFile(MultipartFile file, String rootPath) {
        ...
        // 이 파일을 날짜 별 관리를 위해 날짜별 폴더를 생성
        String newUploadPath = makeDateFormatDirectory(rootPath);

        // 파일의 풀 경로를 생성
        String fullPath = newUploadPath + "/" + newFileName; // 확장자는 OriginalFilename에 포함되어 있다.
        ...
        return fullPath.substring(rootPath.length()); // DB 저장용으로 full-path에서 루트 경로 뺴고 리턴
    }
    ...
    private static String len2(int n) {
        return new DecimalFormat("00").format(n); // int n을 두자리 "00" 패턴으로 바꿔라.
    }
