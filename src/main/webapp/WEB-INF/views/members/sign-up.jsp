...
<body>
  ...
  <script>
    ...
    // 아이디 검사 정규표현식
    const accountPattern = /^[a-zA-Z0-9]{4,14}$/; //^문장 시작 표시, $문장 끝 표시, 영문대소문자, 4~14자 사이만 허용
    ...
    $idInput.onkeyup = e => {
      ...
      const idValue = $idInput.value; // 사용자의 입력값
      // console.log($idValue);

      if (idValue.trim() === '') {
        $idInput.style.borderColor = 'red';
        document.getElementById('idChk').innerHTML = '<b style = "color:red;">아이디는 필수값입니다.</b>';
        checkResultList[0] = false; // 사용자가 제대로 작성했다가 지우는 경우도 있으니 false 값으로 되돌려 놔야 함.
      } else if (!accountPattern.test(idValue)) {
        // 정규표현식의 함수 test()를 통해서 입력값이 패턴에 유효한지 검증
        // 패턴과 일치하는 입력값이면 true, 하나라도 어긋난다면 false
        document.getElementById('idChk').innerHTML = '<b style = "color:red;">아이디는 4~14글자의 영문, 숫자로 작성하세요.</b>';
        ...
        fetch('/members/check/account/' + idValue)
          ...
          .then(flag => {
            if (flag) {
              $idInput.style.borderColor = 'red';
              document.getElementById('idChk').innerHTML = '<b style = "color:red;">중복된 아이디입니다.</b>';
              checkResultList[0] = false;
            } else {
              $idInput.style.borderColor = 'red';
    ...
    document.getElementById('signup-btn').onclick = e => {
      // 5개의 입력칸이 모두 통과되었을 경우 폼을 submit, 동기 방식으로 요청 보낸다.
