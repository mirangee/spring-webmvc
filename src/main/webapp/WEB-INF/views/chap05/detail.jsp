    ...
    <script>
        ...
        function renderPage({
            ...
            for (let i = begin; i <= end; i++) {
                let active = '';
                if (page.pageNo == i) { // 사용자가 선택한 페이지 번호와 현재 페이지 번호가 같다면
                    active = 'p-active'; // 색칠 넣기
                }
            ...
            // 페이지태그 렌더링
            document.querySelector('ul.pagination').innerHTML = tag;
        }

        // 화면에 댓글 태그들을 렌더링하는 함수
        function renderReplies(replyList) { // 댓글 페이징 적용하면 replyList 객체에 count, pageInfo, replies가 담겨 있음
            ...
            if (replies !== null && replies.length > 0) {
                ...
                for (let reply of replies) { // 자바에서 디자인한 ReplyDetailResponseDTO가 여기로 옴
                    // 객체 디스트럭처링(구조분해할당)
                    const {
                        rno,
                        writer,
        ...
        function fetchGetReplies(pageNum = 1) { // 댓글 목록 요청 시 페이지 번호가 전달(전달되지 않으면 기본값 1)
            ...
            fetch(`\${URL}/\${bno}/page/\${pageNum}`) // GET 방식이므로 객체 따로 선언하지 않음
                .then(res => res.json()) // 응답된 JSON 객체를 자바스크립트 객체로 꺼내고
                .then(replyList => { //꺼낸 자바스크립트 객체에서 replyList 꺼내 보기
        ...
        function makePageButtonClickHandler() {
            ...
            $pageUl.onclick = e => {
                // 이벤트 타깃이 a태그(페이지 버튼)가 아니면 href 속성을 가져오지 못하니 타깃 제한
                ...
                fetch(URL, requestInfo)
                    ...
                    .then(res => {
                        ...
                        if (res.status === 200) {
                            ...
                            return res.text(); // Promise 객체가 반환됨
                        } else {
                            alert('입력값에 문제가 있습니다! 입력값을 다시 확인해 보세요!');
                            return res.text();
                        }
                    })
                    .then(data => { // Promise 객체의 내용 꺼내기
                        console.log('응답 성공! ', data);
                        // 댓글 작성자 input과 댓글 내용 text를 지워주자.
                        $replyText.value = '';
                        // $replyWriter.value = ''; 로그인한 회원 이름이 계속 남아있도록 주석처리
        ...
        function makeReplyRemoveClickHandler() {
            ...
            $replyData.onclick = e => {
                ...
                //태그에 reply-id라고 되어 있으면 replyId라고 지목하면 되지만 replyId라고 되어있기 때문에 replyid라고 지목해야 한다.
                    ...
                    */
                    ...
                    fetch(`\${URL}/\${rno}`, {
                        ...
                        .then(res => {
                            if (res.status === 200) {
                                alert('댓글이 삭제 되었습니다');
                                fetchGetReplies(); // 삭제 완료 후 1페이지 댓글 목록 요청이 들어가게 처리
                            } else {
        ...
        function makeReplyModifyClickHandler() {
            ...
            $modBtn.addEventListener('click', e => {
                const payload = {
                    rno: +document.getElementById('modReplyId').value, // value는 문자열이므로 앞에 '+'를 붙여서 정수형으로 할당
                ...
                fetch(URL, requestInfo)
                    .then(res => {
                        if (res.status === 200) {
                            ...
                            document.getElementById('modReplyText').value = ''; //공백 있을 수 있으니 비우기
                            return;
                        }
                    })
                    .then(data => {
                        console.log(data);
                        fetchGetReplies(); // 수정 완료 후 1페이지 댓글 목록 요청이 들어가게 처리
                    })
